...
from donkeycar.parts.object_detector import YoloDetector
...
def drive(
    ...
    if cfg.ENABLE_OBJECT_DETECTION:
        yolo = YoloDetector(
            config_path=cfg.YOLO_CONFIG_PATH,
            weights_path=cfg.YOLO_WEIGHTS_PATH,
            labels_path=cfg.YOLO_LABELS_PATH,
            conf_threshold=cfg.YOLO_CONFIDENCE_THRESHOLD,
            nms_threshold=cfg.YOLO_NMS_THRESHOLD,
            draw_bboxes=True,
            input_size=608,
        ...
        V.add(
            ...
            outputs=["cam/image_array", "detect/objects"],
            threaded=False,
        )
    ...
    #
    V.add(
        Lambda(lambda v: print("web/w1 clicked")),
        inputs=["web/w1"],
        run_condition="web/w1",
    )
    V.add(
        Lambda(lambda v: print("web/w2 clicked")),
        inputs=["web/w2"],
        run_condition="web/w2",
    )
    V.add(
        Lambda(lambda v: print("web/w3 clicked")),
        inputs=["web/w3"],
        run_condition="web/w3",
    )
    V.add(
        Lambda(lambda v: print("web/w4 clicked")),
        inputs=["web/w4"],
        run_condition="web/w4",
    )
    V.add(
        Lambda(lambda v: print("web/w5 clicked")),
        inputs=["web/w5"],
